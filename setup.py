...
def get_platform_specific_include_paths():
    """makes the list of OS-specific include directories"""
    ...
    if system == "windows":
        ...
        return [
            join(JDK_include_path, "linux"),
            join(JDK_source_path, "java.base", "linux", "native", "libjava"),
            join(JDK_source_path, "java.base", "unix", "native", "libjava")
        ]
    elif system == "darwin":
        raise NotImplementedError("Darwin isn't supported yet sorry!")
    
    raise NotImplementedError("OS other than windows and linux aren't currently supported. Feel free to contribute it to the repo if you add your OS!")
...
setup(
    ...
    ext_modules=[Extension(
        ...
        include_dirs=[
            ...
            *get_platform_specific_include_paths()
        ],
        define_macros=[("NO_ZLIB", 1)]
    )]
)
